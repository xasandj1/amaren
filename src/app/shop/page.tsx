                        ...
                        {!products ? (
                            ...
                            products.map((product: Product) => (
                                <div key={product.id} className='flex items-center mb-3 border border-primaryYellow justify-between p-4 xs:flex-col md:flex-row'>
                                    <img src={product.images![0].image} alt="product images" className='w-24 h-24' />
                                    <div className="ml-5 xs:flex xs:flex-col xs:items-center md:flex-col md:items-start xs:ml-0 md:ml-5">
                                        ...
                                        <div className="flex items-center mt-2 gap-2">
                                            <button className='text-sm font-normal text-white bg-primaryYellow rounded p-1 flex items-center justify-center' onClick={() => toFavorite(product)}>
                                                Save in Wishlist
                                            </button>
                                    ...
                                    <div className="ml-5 flex items-center xs:ml-0 xs:mt-3">
                                        <button className='px-4 py-1.5 bg-green-800 text-white border-none rounded cursor-pointer text-lg flex justify-center items-center' onClick={() => plusCount(product.id)}>
                                            <FaPlus />
                                        </button>
                                        <span className='mx-2 w-20 border border-gray-300 text-lg font-bold text-center rounded p-1.5'>
                                            {product.userCount}
                                        </span>
                                        <button className='px-4 py-1.5 bg-[#dd0404] text-white border-none rounded cursor-pointer text-lg flex justify-center items-center' onClick={() => minusCount(product.id, product.userCount)} disabled={product.userCount === 0}>
                                            <FaMinus />
                                        </button>
                                    </div>
